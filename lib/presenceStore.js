...
export async function getOnlineCount() {
  try {
    ...
    await r.zremrangebyscore(KEY, 0, now - ONLINE_WINDOW_MS);
    // On lit toutes les entrées et on filtre celles qui sont en
    // localhost (::1 / 127.0.0.1) — du bruit qui vient des sessions
    // dev locales (qui partagent le même Redis que la prod).
