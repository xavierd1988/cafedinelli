...
//
// Champs par produit :
//   - emoji   : fallback visuel toujours juste (obligatoire)
//   - name    : nom affiché (obligatoire)
//   - search  : keywords pour la recherche Amazon (fallback)
//   - asin    : (optionnel) ID Amazon stable, ex. "B07PVCVBN7". Quand
//               présent, /api/product-image scrape la page /dp/{asin}
//               directement → photo du bon produit, pas un résultat de
//               recherche aléatoire.
//   - image   : (optionnel) URL Amazon CDN hardcodée. Quand présent, le
//               client utilise cette URL directement, AUCUN scrape.
//               Max précision, max performance, max stabilité.
...
export function getDailyProducts(count = 30) {
  ...
  return shuffled.slice(0, count).map((p, i) => ({
    ...
    emoji: p.emoji,
    name: p.name,
    // Optionnels : remontent au client pour bypass scraping quand possible
    asin: p.asin || null,
    image: p.image || null,
    // Lien Amazon : page produit directe si ASIN connu, sinon recherche
    amazonUrl: p.asin
      ? `https://www.amazon.com/dp/${p.asin}`
      : `https://www.amazon.com/s?k=${p.search}`
  }));
}

// Exposé pour le script scripts/cache-product-images.js qui pré-scrape
// tous les produits du pool et imprime des `image: "..."` à coller ici.
export { POOL };
