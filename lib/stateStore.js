...
import { notifyNewOnline } from "./onlineNotifier.js";
  ...
  if (recordPing && ip) {
    ...
    try {
      ...
      if (isNew) {
        ...
        // IMPORTANT : on AWAIT pour garantir que le fetch part avant
        // que la function serverless se termine. Sans await, Vercel
        // edge runtime peut couper le fetch en vol et la notif n'arrive
        // jamais. Import statique pour éviter les soucis de bundling
        // edge runtime avec les imports dynamiques.
        try { await notifyNewOnline(); } catch { /* ignore */ }
      }
    } catch {
