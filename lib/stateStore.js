...
async function buildRawSnapshot() {
  const [seats, regulars, mikeRaw, eyeRaw, taxiRaw, online, secretRoom] =
    await Promise.all([
      safe(getActiveSeats(), []),
 ...
 */
export async function getCafeState(ip, { recordPing = true } = {}) {
  if (recordPing && ip) {
    // Fire-and-forget : la présence ne doit pas bloquer le snapshot.
    recordPresence(ip).catch(() => {});
  }
