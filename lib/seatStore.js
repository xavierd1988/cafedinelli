...
export async function recordSeatMessage({ id, ip, sessionId, nickname, message, persona }) {
  const entry = {
    id,
    ip,
    // sessionId : généré côté client (sessionStorage), persiste au refresh
    // mais disparaît à la fermeture d'onglet. Sert au verrou "1 seat par
    // session" — empêche de changer de siège sans nouvelle session.
...
// Renvoie le siège ACTIF occupé par cette session browser. Utilisé pour
// le verrou "1 seat par session" : tant que cette session existe (refresh
// inclus), elle ne peut pas changer de siège. Seule une fermeture +
// nouvelle ouverture d'onglet (= nouveau sessionId) débloque.
export async function findActiveSeatForSession(sessionId) {
  if (!sessionId || typeof sessionId !== "string") return null;
  const all = await loadAllRaw();
  return all.find((s) => s.sessionId === sessionId && !s.releasedAt) || null;
}

// Marque un siège comme "released" sans l'effacer. Sert de filet de
// sécurité quand un visiteur POST sur un autre siège sans avoir cliqué
// la silhouette (refresh, swap rapide). L'entrée garde son timestamp et
// son message jusqu'à expiration naturelle, mais le verrou IP est libéré.
...
// HARD-delete : utilisé quand le visiteur clique sa silhouette pour
// partir explicitement. L'entrée disparaît tout de suite pour TOUT le
// monde — pas de délai 120s. C'est le comportement "leave fast".
export async function leaveSeatForIp(ip) {
  ...
  try {
    await getRedis().hdel(KEY, String(mine.id));
    return true;
  } catch (err) {
