...
// Libère un seatId (HDEL). Appelé par le POST quand le visiteur change de
// siège — on enlève silencieusement l'ancienne entrée pour qu'il puisse
// poster sur le nouveau sans 409.
export async function releaseSeat(id) {
  if (id === null || id === undefined) return;
  try {
    await getRedis().hdel(KEY, String(id));
  } catch (err) {
    console.warn("seatStore.releaseSeat: Redis indispo", err?.message);
  }
}
